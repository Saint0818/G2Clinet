...
public class EasyJoystick : MonoBehaviour {
	...
	public Texture touchTexture3;
	private int size1 = 210;	
	private int size2 = 160;
	private int size3 = 120;
...
//			GUI.Box(new Rect(0, 0, VirtualScreen.width * 3 / 4, VirtualScreen.height * 3 / 4), "test");
			...
			if ((showTouch && touchTexture!=null && !dynamicJoystick)|| (showTouch && dynamicJoystick && virtualJoystick && touchTexture!=null) 
				...
				{
					if(joystickTouch.x >= Min)
						x = Mathf.Min(joystickTouch.x - Min, Max);
				
					if(joystickTouch.x <= -Min)
						x = Mathf.Max(joystickTouch.x + Min, -Max);

					if(joystickTouch.y >= Min)
						y = Mathf.Min(joystickTouch.y - Min, Max);
					
					if(joystickTouch.y <= -Min)
						y = Mathf.Max(joystickTouch.y + Min, -Max);					
				...
				if(touchTexture3 != null)
				{
					...
					GUI.DrawTexture( new Rect(anchorPosition.x + joystickCenter.x + (x - size3) + (size3 / 2),
					...
					if (!isVirtualTouch) {
						...
						switch (area){
							...
							case DynamicArea.BottomLeft:
								...
								break;
							case DynamicArea.Custom:
								if (gesture.position.y< Screen.height * 0.8f && gesture.position.x< Screen.width * 0.5f){
									virtualJoystick = true;	
									isVirtualTouch = true;
								}
								break;							
