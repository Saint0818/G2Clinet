...
public class RPGViewFrustum : MonoBehaviour {
	...
	public OccultationHandling OccultationHandling = OccultationHandling.TagDependent;
	// Specifies the object layers which get considered inside the view frustum
	public LayerMask OccultingLayers = 1;
	// If "OccultationHandling" is set to "TagDependent", these object tags affect the camera zoom
	...
	*/
	public float CheckForOccultation(Vector3 atPosition, Vector3 target, Camera usedCamera) {
		OccultationHandling handling = OccultationHandling;

		if (handling == OccultationHandling.DoNothing) {
			// All objects inside the view frustum should be ignored, hence there is no occultation => return -1
			return -1;
		}
		...
		ImprovedRaycastAll(_targetPosition, rayDirection, rayDistance, OccultingLayers, handling, ref objectsToFade, ref closestRaycastHits);
		ImprovedRaycastAll(_targetPosition + _shiftUpperLeft, rayDirection, rayDistance, OccultingLayers, handling, ref objectsToFade, ref closestRaycastHits);
		ImprovedRaycastAll(_targetPosition + _shiftUpperRight, rayDirection, rayDistance, OccultingLayers, handling, ref objectsToFade, ref closestRaycastHits);
		ImprovedRaycastAll(_targetPosition + _shiftLowerLeft, rayDirection, rayDistance, OccultingLayers, handling, ref objectsToFade, ref closestRaycastHits);
		ImprovedRaycastAll(_targetPosition + _shiftLowerRight, rayDirection, rayDistance, OccultingLayers, handling, ref objectsToFade, ref closestRaycastHits);
		...
		if (handling == OccultationHandling.TagDependent) {
	...
	*/
	...
	{	
		...
		if (hitArray.Length > 0) {
			...
			if (handling == OccultationHandling.AlwaysZoomIn) {
			...
			} else  if (handling == OccultationHandling.TagDependent) {
	...
	private void CharacterFade(Camera usedCamera) {
		...
		if (OccultationHandling == OccultationHandling.TagDependent && Convert.ToBoolean(OccultingLayers & (1 << gameObject.layer))) {
			// Occultation is tag dependent and the character's layer is part of the occulting layers 
	...
	public OccultationHandling GetOccultationHandling() {
		return OccultationHandling;
	}
	...
	private void OnDrawGizmos() {
		...
		Gizmos.DrawRay(_targetPosition, viewFrustumDirection);
	}
}
