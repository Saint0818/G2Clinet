...
public class BallTrigger : MonoBehaviour
{
    ...
    private bool touchPlayer(Collider other, bool isEnter)
    {
        ...
        if (dir > -1)
        {
            ...
            {
                PlayerBehaviour player = other.gameObject.transform.GetComponentInParent<PlayerBehaviour>();
                if (player)
                {
                    if (isEnter)
                    {
						if (GameController.Get.PassingStealBall(player, dir) || GameController.Get.BallState == GameEnum.EBallState.CanRebound)
