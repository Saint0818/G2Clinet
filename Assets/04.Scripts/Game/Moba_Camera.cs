...
public class Moba_Camera_Settings_Movement {
	...
	public float lockTransitionRate		= 0.05f;
...
public class Moba_Camera : MonoBehaviour {
	...
	private float fourceRotationY = 180;
	...
	void CalculateForceRotation() {
		if (_currentCameraRotation.y != fourceRotationY) {
			float percentage = 1;
			if (rotationTime > 0) {
				percentage = (Time.time - startTime) / rotationTime;
			}
			
			if (percentage > 1)
				percentage = 1;
						
			if (fourceRotationY == 0) {
				_currentCameraRotation.y = 180 - 180 * percentage;
			} else
				_currentCameraRotation.y = fourceRotationY * percentage;
	...
	public float LockRotationY {
		set {
			startTime = Time.time;
			fourceRotationY = value;
			changeInCamera = true;
		}
	}
}
