...
[AddComponentMenu("NGUI/UI/Sprite Animation Delay")]
public class UISpriteAnimationDelay : MonoBehaviour
{
	...
	[HideInInspector][SerializeField] protected float mDelayTime = 0;
	...
	{
		if (mDelay > 0 && mLoop) 
			mDelay -= RealTime.deltaTime;
		else 
			mDelay = 0;

		if (mDelay <= 0 && mActive && mSpriteNames.Count > 1 && Application.isPlaying && mFPS > 0)
		{
