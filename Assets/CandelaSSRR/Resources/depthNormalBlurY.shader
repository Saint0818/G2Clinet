...
//--------------------------------
	...
	half4 centerSample = tex2Dlod(_CameraDepthNormalsTexture, float4(uvflipped,0,0));
	...
	float4 worldnorm 	  = tex2Dlod(_CameraNormalsTexture, float4(uvflipped,0,0));
	...
	for(s=1;s<4;s++)
	{	
		sampleUV		= i.uv + (float2(0,-abs(_MainTex_TexelSize.y)*s)*blurRad);
		float2 sampleUV2		= uvflipped + (float2(0,-abs(_MainTex_TexelSize.y)*s)*blurRad);
		//sampleUV2.y = 1-sampleUV2.y;
		edgeCheck 		= CheckSame(centerSample.xy, DecodeFloatRG(centerSample.zw), tex2Dlod(_CameraDepthNormalsTexture, float4(sampleUV2,0,0)));
	...
	if(worldnorm.w > 0.7)
	{
		for(s=1;s<4;s++)
		{
		 ...
		 float2 sampleUV2		= uvflipped + (float2(0,-_MainTex_TexelSize.y*s)*blurRad);
		 edgeCheck 		= CheckSame(centerSample.xy, DecodeFloatRG(centerSample.zw), tex2Dlod(_CameraDepthNormalsTexture, float4(sampleUV2,0,0)));
		 if(edgeCheck > 0)//SamplePoint Ok..
		 {
	...
	if(worldnorm.w > 0.7)
	{
		for(s=1;s<4;s++)
		{
		 sampleUV		= i.uv + (float2(0,+abs(_MainTex_TexelSize.y)*s)*alphaBlurRad);
		 float2 sampleUV2		= uvflipped - (float2(0,+(_MainTex_TexelSize.y)*s)*blurRad);
		 edgeCheck 		= CheckSame(centerSample.xy, DecodeFloatRG(centerSample.zw), tex2Dlod(_CameraDepthNormalsTexture, float4(sampleUV2,0,0)));
		 if(edgeCheck > 0)//SamplePoint Ok..
		 {
