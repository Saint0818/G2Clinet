...
public class CandelaSSRR : MonoBehaviour
{
	...
	public float FlipReflectionsForMSAA = 0.0f;
	...
	void OnPreRender()
	{
		...
		RenderTexture camRT = RenderTexture.GetTemporary(Screen.width,Screen.height, 24, RenderTextureFormat.RGFloat);
		
		if(UseLayerMask)
		{
		RTcustom_CAMERA.cullingMask ^= (1<<cullingmask);
		}
		...
		if(renderCustomColorMap)
		{
		//========================================================
		RTcustom_CAMERA.backgroundColor = new Color(0,0,0,0);
		RenderTexture camRTcol = RenderTexture.GetTemporary(Screen.width,Screen.height, 24,RenderTextureFormat.ARGB32);
		RTcustom_CAMERA.targetTexture = camRTcol;
		RTcustom_CAMERA.Render();
		camRTcol.SetGlobalShaderProperty("_ColorTextureCustom");	
		RenderTexture.ReleaseTemporary (camRTcol);
		SSRR_MATERIAL.SetFloat("_renderCustomColorMap", 1.0f);
		//========================================================
		}
		else
		{
		SSRR_MATERIAL.SetFloat("_renderCustomColorMap", 0.0f);
		}
		...
		Shader.SetGlobalFloat("_alphaBiasControlSSRR", alphaBiasControlSSRR);	
		RTcustom_CAMERA.backgroundColor = new Color(0,0,0,0);
	...
	void OnRenderImage (RenderTexture source, RenderTexture destination)
	{
		...
		Shader.SetGlobalFloat("_FlipReflectionsMSAA", FlipReflectionsForMSAA);
