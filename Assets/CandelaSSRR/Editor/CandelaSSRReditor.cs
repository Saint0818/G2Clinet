...
public class CandelaSSRReditor : Editor
{
	...
	public override void OnInspectorGUI() 
	{
		...
		//===============================
		//This Is Only Important In Forward Render Mode With DirectX And Multisampling Enabled
		//===============================
		
		GUILayout.Space(3);
		GUI.contentColor = new Color(0.7f,0.7f,1.0f,1.0f);
		GUILayout.Label("Only Flip Reflections For MSAA Support",EditorStyles.boldLabel);
		GUILayout.Space(3);
		GUI.contentColor = Color.white;
		GUILayout.BeginVertical(back1);
		
		bool flipReflectionsTmp = false;
		if(cdla.FlipReflectionsForMSAA > 0.0f) flipReflectionsTmp = true;
		if(EditorGUILayout.Toggle("Flip Reflections", flipReflectionsTmp))
			cdla.FlipReflectionsForMSAA = 1.0f;
		else
			cdla.FlipReflectionsForMSAA = 0.0f;
		
		GUILayout.EndVertical();
		//================================
		//OPTIONAL CULLING MASK
		//================================
		GUILayout.Space(5);
		GUI.contentColor = new Color(0.7f,0.7f,1.0f,1.0f);
		...
		//EditorGUIUtility.LookLikeInspector();
		cdla.cullingmask 	   = EditorGUILayout.LayerField("Layer Mask", cdla.cullingmask);
		//EditorGUIUtility.LookLikeControls();
		GUILayout.Space(5);
		//================================
		//Inorder To Be Able To Completely Exclude Objects From Casting And Receiving Reflections
		//custom Color Map Must be rendered
		//================================
		GUI.contentColor = new Color(0.7f,0.7f,1.0f,1.0f);
		GUILayout.Label("Enable Complete SSRR Exclusion Mode ",EditorStyles.boldLabel);
		GUI.contentColor = Color.white;
		cdla.renderCustomColorMap      = EditorGUILayout.Toggle("CompleteExcludeMode", cdla.renderCustomColorMap);
		
		//================================
		//Allow Alpha Bias Control For Compatibility With Other Packages
		//================================
		GUILayout.Space(5);
		GUI.contentColor = new Color(0.7f,0.7f,1.0f,1.0f);
		GUILayout.Label("Optional Alpha Bias Control (In Forward Render Only)",EditorStyles.boldLabel);
		GUI.contentColor = Color.white;
		cdla.alphaBiasControlSSRR  = EditorGUILayout.Slider("AlphaBias", cdla.alphaBiasControlSSRR, 1.0f, 10.0f);
		GUILayout.Space(5);
		//================================
