...
public class GEBasketPosition : GEBase {
	...
	void OnGUI() {
			...
			for (int i=0; i<allBasketAnimationClip.Count; i++) {
				if(allBasketAnimationClip.Count > 0){
					...
					if(isMatch) 
						...
						GUI.Label(new Rect(300, (20 * i)+ 150, 150, 20), allBasketAnimationClip[i].name, StyleLabel);
				} else
					GUI.Label(new Rect(300, (20 * i)+ 150, 150, 20), allBasketAnimationClip[i].name);
			}
			GUI.EndScrollView ();
		...
		if(GUI.Button(new Rect(200, 550, 200, 20), "Save"))
			OnSave();
		if(isSave)
			GUI.Label(new Rect(200, 575, 200, 20), "Save Success", StyleLabel);
	}
