...
public class AddAutoSubscribe
{
	...
	{
		if (Selection.gameObjects.Length > 0) {
			Material mat = Resources.Load<Material>("Materials/DynamicFont");
			for (int i = 0; i < Selection.gameObjects.Length; i ++) {
				...
				if (labs != null && labs.Length > 0) {
					for (int j = 0; j < labs.Length; j ++) {
						labs[j].depth = 200;
						labs[j].material = mat;
						if (labs[j].fontSize >= 100)
							labs[j].fontSize = 100;
						else
						if (labs[j].fontSize >= 84)
							labs[j].fontSize = 84;
						else
						if (labs[j].fontSize >= 72)
							labs[j].fontSize = 72;
						else
						if (labs[j].fontSize >= 60)
							labs[j].fontSize = 60;
						else
						if (labs[j].fontSize >= 48)
							labs[j].fontSize = 48;
						else
						if (labs[j].fontSize >= 36)
							labs[j].fontSize = 36;
						else
						if (labs[j].fontSize >= 32)
							labs[j].fontSize = 32;
						else
						if (labs[j].fontSize >= 28)
							labs[j].fontSize = 28;
						else
							labs[j].fontSize = 24;
					}
